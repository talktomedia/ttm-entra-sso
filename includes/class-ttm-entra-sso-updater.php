 ...
 * updater plugin required on any of the 80 client sites. WordPress core
 * already provides everything this needs (pre_set_site_transient_update_plugins
 * to advertise an update, upgrader_source_selection to fix up the downloaded
 * folder name); we just have to feed it the right data.
 *
 * To publish a new version: bump the Version header in ttm-entra-sso.php,
 * commit, then `git tag 1.2.0 && git push origin 1.2.0` (a "v" prefix is
 * fine too, it's stripped). Sites pick it up on WordPress's normal update
 * schedule, or immediately via "Check Again" on Dashboard -> Updates.
 */
final class TTM_Entra_SSO_Updater
{
     ...
     * admin loading pages repeatedly doesn't hammer the GitHub API.
     *
    ...
    /**
     * GitHub's tag zip extracts to "{repo}-{ref}", not this plugin's actual
     * installed folder name - without this, WordPress would install the
     * update as a second, differently-named plugin instead of replacing
     * this one in place.
     *
     * @param string $source
    ...
    public function fix_folder_name(string $source, string $remote_source, $upgrader, $hook_extra = null)
    {
        ...
        if (!is_array($hook_extra) || ($hook_extra['plugin'] ?? '') !== $this->pluginBasename) {
            return $source; // Not this plugin's update - leave it alone.
        }
